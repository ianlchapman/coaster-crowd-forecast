...
def score_is_open(test: pd.DataFrame, pred: pd.DataFrame, cutoff: pd.Timestamp) -> None:
    ...
    g = pd.DataFrame({"bucket": pd.cut(days_ahead, bins=bins, labels=labels), "hit": is_open == actual_open, "actual_open": actual_open})
    print("by days_ahead-from-cutoff bucket:")
    for b, gg in g.groupby("bucket", observed=True):
        print(f"  {b:>7}: n={len(gg):6d}  accuracy={gg['hit'].mean():.4f}  actual_open_rate={gg['actual_open'].mean():.3f}")
...
def score_hours(test: pd.DataFrame, pred: pd.DataFrame) -> None:
    ...
    exact_closes = (pred.loc[both, "closes"].fillna("") == test.loc[both, "closes"].dt.strftime("%H:%M").fillna("")).mean()

    print(f"\n=== opens/closes (StatusModel, n={both.sum()} actually-open days) ===")
    print(f"opens : MAE={opens_err.mean():.1f}min  median={opens_err.median():.1f}min  exact={exact_opens:.3f}  within30min={(opens_err <= 30).mean():.3f}")
    print(f"closes: MAE={closes_err.mean():.1f}min  median={closes_err.median():.1f}min  exact={exact_closes:.3f}  within30min={(closes_err <= 30).mean():.3f}")
...
if __name__ == "__main__":
    ...
    test["opens"] = pd.to_datetime(raw.set_index(["park_id", "date"])["opens"].reindex(pd.MultiIndex.from_frame(test[["park_id", "date"]])).to_numpy(), format="%H:%M", errors="coerce")
    test["closes"] = pd.to_datetime(raw.set_index(["park_id", "date"])["closes"].reindex(pd.MultiIndex.from_frame(test[["park_id", "date"]])).to_numpy(), format="%H:%M", errors="coerce")
    ...
    print("\ntop is_open features (% gain):", (100 * imp / imp.sum()).sort_values(ascending=False).head(8).round(1).to_dict())
    imp = pd.Series(model.closes.booster_.feature_importance("gain"), index=HOURS_FEATURES)
    print("top closes features (% gain):", (100 * imp / imp.sum()).sort_values(ascending=False).head(8).round(1).to_dict())

    pred.assign(actual_is_open=test["is_open"].to_numpy()).to_csv(out_dir() / "opening_hours_model_eval.csv", index=False)
